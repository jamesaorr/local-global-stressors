...
* **time**: average of water quality samples taken during the preceding summer (only between 10 and 3pm)
* **space**: water quality site must be in the same river and within 1km
...
Repeat this for all years going back to 2005. 
...
data_metrics <- read.csv("data/eco/inverts/INV_OPEN_DATA_METRICS_2022-02-01.csv", 
                     header = T)
...
data_site <- data_metrics %>%
  ...
  left_join(data_site, ., by = "SITE_ID") %>%
  select(-SAMPLES_PER_SITE) %>%
  rename(SAMPLES_PER_SITE = SAMPLES_PER_SITE2) %>%
...
wq_data <- wq_data %>%
  ...
  select(-c(ymd_hms, month, hours, sample.sampleDateTime)) %>%
...
wq_data <- wq_data %>%
  ...
  group_by(ID) %>%
  summarise_at(names(wq_data[2:15]), mean, na.rm = TRUE) %>%
...
## This suggests that I can use discharge! 
```


The flow data is not close enough to the invertebrate data. For 2010 we have 66 sites with inverts and water quality - but if we want flow as well this goes down to 9. Average flow in these 9 sites correlates very well with the discharge varible in the invert data - so I will use that data instead.
...
```{r}
invert_taxa <- read.csv("data/eco/inverts/INV_OPEN_DATA_TAXA_2022-02-01.csv", 
                     header = T)
...
invert_taxa <- subset(invert_taxa, invert_taxa$TAXON_TYPE == "Other Macroinvertebrates")
...
Get abundances for the seven higher taxa showing the greatest change among classes in the Vaughan and Goetteli paper: 
*Bivalvia, Hirudinea, Isopoda, Ephemeroptera, Plecoptera, Trichoptera, Diptera*


Also calculate macroinvertebrate richness

And get the total abundance of macroinvertebrates and also the richness of the EPT and the abundance of EPT. 

So what I need now is to summarize the data so that for each analysis ID there is the abundance of the groups I'm interested in and the total macroinvertebrate richness (number of rows per analysis ID).
...
invert_taxa_abundances <- select(invert_taxa_abundances, 
                                 c("annelid",
                                   "crustacean",
